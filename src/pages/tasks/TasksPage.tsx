...
const TaskCard: React.FC<TaskCardProps> = ({ task, onWorkflowClick, onDetailsClick }) => {
  ...
  return (
    <div
      ...
      className={`rounded-2xl p-3.5 flex flex-col border transition-all active:scale-[0.98] cursor-pointer mb-1 ${
        isDarkMode ? 'bg-[#1a1a1c] border-gray-800' : 'bg-white border-gray-100 shadow-sm'
      }`}
    >
    ...
    <Page className={`flex flex-col w-full h-full relative pb-[90px] ${isDarkMode ? 'bg-[#121212]' : 'bg-[#fcfdff]'}`}>
      ...
      <div className={`flex items-center justify-between px-5 pt-6 pb-2 sticky top-0 z-[100] ${isDarkMode ? 'bg-[#121212]' : 'bg-[#fcfdff]'}`}>
        ...
        <div className="flex items-center gap-1">
          ...
          >
            {isLoading ? <LoadingOutlined className="text-[#1e3ba1] text-[18px]" /> : <RocketOutlined className={`text-[18px] rotate-180 ${isDarkMode ? 'text-gray-400' : 'text-[#1e3ba1]'}`} />}
          </button>
      ...
      <div className="px-5 mt-6">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            {(tasks.some(t => t.status === 'TO DO' || t.status === 'IN PROGRESS') || tab === 'UNASSIGNED') && (
                <div>
