...
import { useRegisterMutation, useZaloLoginMutation } from '@/hooks/useAuth';
...
const RegisterPage: React.FC = () => {
  ...
  const handleRegister = () => {
    ...
    */
    ...
    // BYPASS VÀO LUÔN
    login(
      { id: 'dev-dummy', email: 'admin@renren.com', roles: ['ADMIN'], name: name || 'User' },
      'dummy-access-token',
      'dummy-refresh-token'
    );
    navigate('/dashboard');
  ...
  const handleZaloLogin = async () => {
    try {
      ...
      */
      ...
      console.error(error);
      alert('Không thể liên kết với Zalo Mini App. Hãy thử lại.');
    }
  };
    ...
    <>
      ...
      <div className="flex flex-col h-screen w-full bg-white relative max-w-[390px] mx-auto overflow-y-auto hide-scrollbar">
        ...
        <div className="h-[56px] flex items-center justify-between px-4 bg-white shrink-0">
          ...
          <h1 className="text-[18px] font-semibold text-gray-900">Đăng ký</h1>
        ...
        <div className="flex flex-col items-center w-full px-5 pt-6 pb-8 bg-white shrink-0">
          ...
          <div className="w-full mb-5 shrink-0">
            ...
            <div className="flex items-center w-full h-[52px] border border-gray-300 rounded-[12px] px-3.5 focus-within:border-[#1E40AF] focus-within:ring-1 focus-within:ring-[#1E40AF] transition-colors bg-white overflow-hidden">
              ...
              />
              {name.length > 0 && (
                <button onClick={() => setName('')} className="p-1 outline-none">
          ...
          <div className="w-full mb-8 shrink-0">
            ...
            <div className="flex items-center w-full h-[52px] border border-gray-300 rounded-[12px] px-3.5 focus-within:border-[#1E40AF] focus-within:ring-1 focus-within:ring-[#1E40AF] transition-colors bg-white overflow-hidden">
              <img src="/icons/lock.svg" alt="confirm-password-icon" className="w-5 h-5 mr-2.5 brightness-0" />
              <input
              ...
              />
              <button onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="p-1 ml-2 outline-none">
                <img
                  src={showConfirmPassword ? "/icons/eye.svg" : "/icons/eye-slash.svg"}
          ...
          <button
            ...
            disabled={name.length < 2 || phone.length < 5 || password.length < 3 || password !== confirmPassword}
            className={`w-full h-[52px] rounded-[12px] font-semibold text-[16px] flex items-center justify-center transition-all shrink-0 outline-none ${name.length >= 2 && phone.length >= 5 && password.length >= 3 && password === confirmPassword
          ...
          >
            ...
            <span>{zaloLoginMutation.isPending ? 'Đang kết nối...' : 'Đăng ký bằng Zalo'}</span>
          </button>

          {/* LIÊN KẾT ĐĂNG NHẬP */}
          <div className="mt-8 flex items-center justify-center gap-1 text-[15px] shrink-0">
            <span className="text-gray-500">Đã có tài khoản?</span>
            <button onClick={() => navigate('/login')} className="text-[#1E40AF] font-semibold active:opacity-70 outline-none bg-transparent p-0">Đăng nhập ngay</button>
          </div>
