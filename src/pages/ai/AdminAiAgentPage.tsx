...
export const AdminAiAgentPage: React.FC = () => {
    ...
    return (
        <Page className={`flex flex-col h-screen ${isDarkMode ? 'bg-[#121212]' : 'bg-[#f4f5f8]'}`}>
            <Header title="Trợ Lý AI Kế Toán/Quản Trị" showBackIcon />
            
            {/* Chat Area */}
            <div className="flex-1 overflow-y-auto px-4 py-6 mt-12 mb-[160px] scroll-smooth">
                        ...
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`flex max-w-[85%] gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                <Avatar 
                                    ...
                                    className={`${msg.role === 'user' ? 'bg-blue-500' : 'bg-gradient-to-r from-purple-500 to-indigo-500'}`}
                                />
                    ...
                    {isTyping && (
                        <div className="flex justify-start">
                            <div className="flex max-w-[85%] gap-2 flex-row">
                                <Avatar icon={<RobotOutlined />} className="bg-gradient-to-r from-purple-500 to-indigo-500" />
            ...
            {/* Input Area */}
            <div 
                className={`fixed left-0 right-0 p-4 border-t z-50 ${isDarkMode ? 'bg-[#1a1a1c] border-gray-800' : 'bg-white border-gray-200'}`}
            ...
            >
                ...
                <div className="flex gap-2">
                    <Input 
                        ...
                        className={`rounded-full shadow-sm ${isDarkMode ? 'bg-gray-800 border-gray-700 text-white placeholder-gray-500' : ''}`}
                        size="large"
                    />
                    <Button 
                        type="primary" 
